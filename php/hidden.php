...
// Check of de gebruiker heeft betaald voor pagina 1
$has_paid_page1 = isset($_SESSION['has_paid_page1']) ? $_SESSION['has_paid_page1'] : false;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($crypto_wallet)) {
        // Hier zou je de betaling kunnen verwerken (bijv. API-aanroep naar een betalingsgateway)
        // Markeer de betaling als voltooid voor pagina 1 in de sessie
        $_SESSION['has_paid_page1'] = true;

        // Redirect om formulier opnieuw indienen te voorkomen
        header("Location: " . $_SERVER['REQUEST_URI']);
        exit; // Zorg ervoor dat je hier exit() aanroept
    } else {
...
<div id="main-content" class="<?= (!$has_paid_page1 && $paywall_enabled) ? 'blurred' : '' ?>">
...
<?php if (!$has_paid_page1 && $paywall_enabled): ?>
